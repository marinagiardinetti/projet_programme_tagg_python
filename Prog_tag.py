...
Le programme demandé doit permettre d'enregistrer les coordonnées des rectangles de sélection dans un fichier
extérieur .json. Je n'ai pas réussi à finir cette partie; j'ai quand même laissé mon "brouillon", l'avancement
à la fin de mon code pour que vous puissiez voir la manière par laquelle je m'y suis prise.
Ainsi, tous les boutons fonctionnenent, sauf le bouton sauvegarder.



=> à changer
    - l'image qui s'affiche à la taille du canevas
   - fichier de labels qui prends en compte les indications récoltées dans les textes
   - affilier label à sélection
   - sortie en un fichier txt avec les labels séléctionnés
